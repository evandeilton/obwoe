...
private:
  // Recursive implementation to concatenate arguments
  void log_impl() {}
...
// Macros for easier logger usage
...
struct BinInfo {
  ...
  std::atomic<int> pos_count;
  std::atomic<int> neg_count;
  std::atomic<double> weight_sum;
  std::atomic<double> pos_weight;
  std::atomic<double> neg_weight;
  ...
  total_count(0), pos_count(0), neg_count(0),
  weight_sum(0), pos_weight(0), neg_weight(0),
  ...
  BinInfo& operator=(const BinInfo& other) {
    if (this != &other) {
      ...
      categories = other.categories;
      total_count.store(other.total_count.load());
      pos_count.store(other.pos_count.load());
      neg_count.store(other.neg_count.load());
      weight_sum.store(other.weight_sum.load());
      pos_weight.store(other.pos_weight.load());
      neg_weight.store(other.neg_weight.load());
      woe = other.woe;
  ...
  // void increment_counts(int total = 1, int pos = 0, int neg = 0, double weight = 1.0) {
  //   total_count.fetch_add(total);
  //   pos_count.fetch_add(pos);
  //   neg_count.fetch_add(neg);
  //   weight_sum.fetch_add(weight);
  //   if (pos > 0) pos_weight.fetch_add(weight);
  //   if (neg > 0) neg_weight.fetch_add(weight);
  // }
  ...
  void increment_counts(int total = 1, int pos = 0, int neg = 0, double weight = 1.0) {
    ...
    double current_weight_sum = weight_sum.load();
    while (!weight_sum.compare_exchange_weak(current_weight_sum, current_weight_sum + weight));
    
    if (pos > 0) {
      double current_pos_weight = pos_weight.load();
      while (!pos_weight.compare_exchange_weak(current_pos_weight, current_pos_weight + weight));
    }
    if (neg > 0) {
      double current_neg_weight = neg_weight.load();
      while (!neg_weight.compare_exchange_weak(current_neg_weight, current_neg_weight + weight));
    }
  }
};
...
public:
  double calculate(
    ...
    std::vector<double> norm_p = p_dist;
    std::vector<double> norm_q = q_dist;
    
    double sum_p = std::accumulate(norm_p.begin(), norm_p.end(), 0.0);
    double sum_q = std::accumulate(norm_q.begin(), norm_q.end(), 0.0);
    
    if (sum_p > constants::EPSILON) {
      for (auto& p : norm_p) p /= sum_p;
    }
    if (sum_q > constants::EPSILON) {
      for (auto& q : norm_q) q /= sum_q;
    }
    ...
    case types::DivergenceMethod::L2_NORM: {
      ...
      for (size_t i = 0; i < norm_p.size(); ++i) {
        sum_squares += std::pow(norm_p[i] - norm_q[i], 2);
      }
    ...
    case types::DivergenceMethod::KULLBACK_LEIBLER: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      if (norm_p[i] > constants::EPSILON && norm_q[i] > constants::EPSILON) {
        result += norm_p[i] * safe_log(safe_divide(norm_p[i], norm_q[i]));
      }
    }
      break;
    }
    case types::DivergenceMethod::J_DIVERGENCE: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      if (norm_p[i] > constants::EPSILON && norm_q[i] > constants::EPSILON) {
        result += (norm_p[i] - norm_q[i]) * safe_log(safe_divide(norm_p[i], norm_q[i]));
      }
    }
      break;
    }
    case types::DivergenceMethod::HELLINGER: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      double sqrt_p = std::sqrt(norm_p[i]);
      double sqrt_q = std::sqrt(norm_q[i]);
    ...
    case types::DivergenceMethod::TOTAL_VARIATION: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      result += std::abs(norm_p[i] - norm_q[i]);
    }
      result = 0.5 * result;
      break;
    }
    ...
    case types::DivergenceMethod::CHI_SQUARE: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      double sum_pq = norm_p[i] + norm_q[i];
      if (sum_pq > constants::EPSILON && 
          norm_p[i] > constants::EPSILON && 
          norm_q[i] > constants::EPSILON) {
        result += safe_divide(std::pow(norm_p[i] - norm_q[i], 2) * sum_pq, 
                              norm_p[i] * norm_q[i]);
      }
    }
      break;
    ...
    case types::DivergenceMethod::JENSEN_SHANNON: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      double m = 0.5 * (norm_p[i] + norm_q[i]);
      if (norm_p[i] > constants::EPSILON && m > constants::EPSILON) {
        result += norm_p[i] * safe_log(safe_divide(norm_p[i], m));
      }
      if (norm_q[i] > constants::EPSILON && m > constants::EPSILON) {
        result += norm_q[i] * safe_log(safe_divide(norm_q[i], m));
      }
    }
      result = 0.5 * result;
      break;
    ...
    case types::DivergenceMethod::LINF_NORM: {
      for (size_t i = 0; i < norm_p.size(); ++i) {
      result = std::max(result, std::abs(norm_p[i] - norm_q[i]));
    }
      break;
    }
    default:
    ...
    if (l1_reg > 0) {
      double l1_penalty = 0;
      for (size_t i = 0; i < norm_p.size(); ++i) {
        l1_penalty += std::abs(norm_p[i]) + std::abs(norm_q[i]);
      }
    ...
    if (l2_reg > 0) {
      double l2_penalty = 0;
      for (size_t i = 0; i < norm_p.size(); ++i) {
        l2_penalty += norm_p[i] * norm_p[i] + norm_q[i] * norm_q[i];
      }
...
public:
  ...
  static void calculate_bin_metrics(
    ...
    // Calculate totals with thread-safe reduction
    double total_pos = 0, total_neg = 0;
#pragma omp parallel for reduction(+:total_pos,total_neg) if(config.use_parallel && bins.size() > constants::PARALLEL_THRESHOLD)
    for (size_t i = 0; i < bins.size(); ++i) {
      total_pos += bins[i].pos_weight.load();
      total_neg += bins[i].neg_weight.load();
    }
    ...
    std::unique_ptr<ConfidenceIntervalCalculator> ci_calc;
    if (config.compute_confidence_intervals) {
      ci_calc = std::make_unique<ConfidenceIntervalCalculator>();
    }
    
    // Calculate metrics for each bin with proper parallelization
#pragma omp parallel if(config.use_parallel && bins.size() > constants::PARALLEL_THRESHOLD)
{
#pragma omp for
  for (size_t i = 0; i < bins.size(); ++i) {
    auto& bin = bins[i];
    ...
    if (config.compute_confidence_intervals && ci_calc) {
...
// Sequential calculation for cumulative metrics to maintain order
for (size_t i = 0; i < bins.size(); ++i) {
  auto& bin = bins[i];
...
private:
  // Function to detect if a variable is binary
  ...
  // Function to create bins for binary variables
  std::vector<BinInfo> create_binary_bins(const std::vector<DataPoint>& data) {
    ...
    // Create two bins for the unique values
    BinInfo bin0, bin1;
    bin0.id = 1;
    bin1.id = 2;
    ...
    for (const auto& dp : data) {
      if (!dp.is_missing && !dp.is_special && std::isfinite(dp.numeric_value)) {
        if (dp.numeric_value == val0) {
          bin0.increment_counts(1, dp.target > 0.5 ? 1 : 0, dp.target <= 0.5 ? 1 : 0, dp.weight);
        } else if (dp.numeric_value == val1) {
          bin1.increment_counts(1, dp.target > 0.5 ? 1 : 0, dp.target <= 0.5 ? 1 : 0, dp.weight);
        }
      }
    }
  ...
  std::vector<BinInfo> create_numeric_bins(
    ...
    // Create quantile-based bins - adjusted to respect max_bins
    int n_bins = std::min(max_prebins, static_cast<int>(clean_data.size()));
    n_bins = std::max(n_bins, config.min_bins); // Ensure at least min_bins
    ...
    // Assign data points to bins with thread-safe operations
#pragma omp parallel for if(config.use_parallel && clean_data.size() > constants::PARALLEL_THRESHOLD)
...
#pragma omp critical
{
  bins[bin_idx].increment_counts(1, dp->target > 0.5 ? 1 : 0, dp->target <= 0.5 ? 1 : 0, dp->weight);
}
      }
    }
  ...
  std::vector<BinInfo> create_categorical_bins(
    ...
    // Group by category with thread-safe operations
    ...
    for (const auto& [category, stats] : category_stats) {
      ...
      if (category_ratio < config.rare_category_threshold) {
        ...
        rare_bin.increment_counts(
          std::get<0>(stats),
          std::get<2>(stats),
          std::get<0>(stats) - std::get<2>(stats),
          std::get<1>(stats)
        );
      } else {
...
private:
  // Check if bins represent a binary variable
  ...
  std::vector<BinInfo> dynamic_programming_optimization(
    ...
    // If already binary, don't optimize
    ...
    for (size_t i = 0; i < partition_points.size(); ++i) {
      ...
      for (int j = start; j <= end && j < static_cast<int>(initial_bins.size()); ++j) {
        merged_bin.increment_counts(
          initial_bins[j].total_count.load(),
          initial_bins[j].pos_count.load(),
          initial_bins[j].neg_count.load(),
          initial_bins[j].weight_sum.load()
        );
  ...
  std::vector<BinInfo> simulated_annealing_optimization(
    ...
    for (int iter = 0; iter < max_iterations && temperature > 0.1; ++iter) {
      ...
      if (bins.size() > 1) {
        ...
        neighbor_bins[merge_idx].increment_counts(
  ...
  std::vector<BinInfo> greedy_merge_optimization(
    ...
    while (static_cast<int>(bins.size()) > config.max_bins && 
      ...
      // Only merge if it doesn't violate min_bins
      if (best_merge_idx >= 0 && 
          static_cast<int>(bins.size()) > config.min_bins) {
        ...
        bins[best_merge_idx].increment_counts(
    ...
    // Ensure it respects min_bins and max_bins
    if (static_cast<int>(bins.size()) < config.min_bins && !initial_bins.empty()) {
      // If we have fewer bins than the minimum required, use initial bins
    ...
    // If we still have more bins than the maximum allowed, force reduction
    while (static_cast<int>(bins.size()) > config.max_bins && bins.size() > 1) {
      // Find the pair of adjacent bins with the smallest IV loss
      int best_merge_idx = -1;
      double min_loss = INFINITY;
...
public:
  std::vector<BinInfo> optimize(
    ...
    // If binary, return directly without optimization
    ...
    // If already within bounds, return directly
    if (static_cast<int>(initial_bins.size()) >= config.min_bins && 
...
public:
  static std::vector<DataPoint> handle_missing_values(
    ...
    if (config.missing_policy == "remove") {
      ...
      for (const auto& dp : data_points) {
        if (dp.is_missing) {
          missing_bin.increment_counts(1, dp.target > 0.5 ? 1 : 0, dp.target <= 0.5 ? 1 : 0, dp.weight);
      ...
      for (const auto& dp : data_points) {
        if (dp.is_missing) {
          ...
          sbin.increment_counts(1, dp.target > 0.5 ? 1 : 0, dp.target <= 0.5 ? 1 : 0, dp.weight);
        } else {
          processed_data.push_back(dp);
        }
      }
...
List process_feature_dmiv(
  ...
  // Store original values for transformation
...
List dmiv_woe(
 ...
 // Create timestamp string properly
